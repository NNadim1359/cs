 ...
 * @param array   $quantity_list   Quantity list from fn_warehouses_aggregate_quantities
 * @param string  $reset_inventory Option for reseting all quantities on mentioned warehouses
 *
 */
function fn_warehouses_exim_set_product_quantities($product_id, $quantity_list, $reset_inventory)
{
    if (!$quantity_list || !is_array($quantity_list)) {
        return;
    }

    if (isset($reset_inventory) && YesNo::toBool($reset_inventory)) {
        db_query("UPDATE ?:warehouses_products_amount SET amount = 0 WHERE warehouse_id IN (?n)", array_keys($quantity_list));
    }
    ...
    foreach ($quantity_list as $warehouse_id => $quantity) {
        $product_stock->setAmountForWarehouse($warehouse_id, $quantity);
    }
