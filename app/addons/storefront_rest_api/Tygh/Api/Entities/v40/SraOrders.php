...
class SraOrders extends Orders
{
    ...
    public function index($id = 0, $params = [])
    {
        ...
        if ($id && !empty($result['data'])) {
            ...
            $this->setProductsIcons($result['data']['product_groups'], $params['icon_sizes']);
            ...
            foreach ($result['data']['orders'] as &$order) {
                ...
                if ($this->safeGet($params, 'get_orders_data', false)) {
                    $this->setProductsIcons($order['product_groups'], $params['icon_sizes']);
                }
            }
            unset($order);
        }
     ...
     * Sets products icons
     *
     * @param array $product_groups Products groups list
     * @param array $icon_sizes     Icon sizes params
     */
    protected function setProductsIcons(array &$product_groups, $icon_sizes)
    {
