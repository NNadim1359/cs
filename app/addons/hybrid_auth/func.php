...
function fn_settings_variants_addons_hybrid_auth_icons_pack()
{
    ...
    $icons_packs = $theme->getDirContents(array(
...
function fn_hybrid_auth_get_providers_list($active = true)
{
    ...
    foreach ($providers_list as $provider_id => $provider) {
        ...
        $providers_list[$provider_id]['icon'] = fn_get_hybrid_auth_provider_icon($provider_id);
    }
...
function fn_hybrid_auth_link_profile($auth_data, $provider)
{
    if (!fn_hybrid_auth_is_exist($auth_data)) {
        if (!empty(Tygh::$app['session']['auth']['user_id'])) {
            fn_hybrid_auth_link_provider(Tygh::$app['session']['auth']['user_id'], $auth_data->identifier, $provider);
        }
    } else {
        fn_set_notification('W', __('notice'), __("hybrid_auth.text_user_is_already_link"));
    }
