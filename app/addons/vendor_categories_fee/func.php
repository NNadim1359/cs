 ...
 * @param Tygh\Tools\Formatter $formatter Formatter to format commissions
 *
 * @return array
 */
function fn_vendor_categories_fee_get_included_product_taxes_based_on_subtotal($subtotals, $taxes, Formatter $formatter = null)
{
...
function fn_vendor_categories_fee_vendor_plans_calculate_commission_for_payout_post($order_info, $company_data, &$payout_data)
{
    ...
    if (Registry::get('addons.vendor_plans.include_shipping') == 'Y') {
        ...
        $shipping_fee = (float) $formatter->asPrice($shipping_fee, CART_PRIMARY_CURRENCY, false, false);
 ...
 * @return mixed
 */
function fn_vendor_categories_fee_calculate_payout($order_total, $payout_data, $products, $main_categories_fee, $parent_categories_fee, $payouts_history, Formatter $formatter = null)
{
 ...
 * @param \Tygh\Tools\Formatter|null $formatter Formatter to use.
 *                                              When null is passed, round with precision of 2 will be used
 *
 * @return float
 */
function fn_vendor_categories_fee_format_amount($amount, Formatter $formatter = null)
{
    if ($formatter !== null) {
        return (float) $formatter->asPrice($amount, CART_PRIMARY_CURRENCY, false, false);
    }
